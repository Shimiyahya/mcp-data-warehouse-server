...
An MCP server that exposes a small **treasury & payments warehouse** (DuckDB) to an LLM as MCP **resources**, **tools**, and a **prompt**, so a model like Claude can plan multi-step analytical SQL and reason over the results while every query goes through a read-only safety layer.

- **What it is:** a read-only MCP server over DuckDB, with the schema and data dictionary as resources plus `list_tables` / `describe_table` / `query` tools.
...
- **What's interesting:** the questions require the model to discover the schema and plan three- and four-way joins (invoices ↔ payments ↔ FX ↔ counterparties) on its own.
...
uv run pytest -q                     # runs the test suite, no API key needed
...
A fully **synthetic, deterministic** warehouse for a fictional company, _Northwind Pay Ltd_. No real entities and no PII; it regenerates byte-identically from `scripts/seed.py` (`random.seed(42)`). Money is stored as **integer minor units** (pennies/cents) to avoid float drift; currencies are ISO-4217; FX is keyed by date so "in GBP" questions take multiple steps to answer.
...
| Tool | Args | Behaviour |
| --- | --- | --- |
| `list_tables` | none | Lists the allow-listed tables with row-count estimates. |
...
Here is exactly what protects the database:
...
- **SELECT/WITH only, single statement.** Every query is parsed to an AST with [`sqlglot`](https://github.com/tobymao/sqlglot); anything that isn't a single top-level `SELECT`/`WITH`/set-op is rejected. This blocks the stacked-statement attack (e.g. `SELECT 1; DROP TABLE x`) that has bitten real database MCP servers.
